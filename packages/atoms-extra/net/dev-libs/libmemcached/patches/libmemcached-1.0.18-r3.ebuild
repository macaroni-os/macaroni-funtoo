...
src_prepare() {
	...
	epatch "${FILESDIR}/libmemcached-fix-automake-build.patch"
	sed -i '6i CFLAGS = @CFLAGS@' Makefile.am
	sed -e "/_APPEND_COMPILE_FLAGS_ERROR(\[-fmudflapth\?\])/d" -i m4/ax_harden_compiler_flags.m4
	# Disable documentation for now. It broken with our version of sphinx.
	sed -i -e 's|include docs.*||g'  Makefile.am
	eautoreconf
}
